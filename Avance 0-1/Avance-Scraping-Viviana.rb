...
class Pregunta
  attr_accessor :titulo, :autor, :edad, :link, :fecha, :respuestas, :imagen

  def initialize(titulo, autor, edad, link, fecha, respuestas, imagen)
    @titulo = titulo
    @autor = autor
    @edad = edad
    @link = link
  ...
  def guardar
    CSV.open('viviana_questions.csv', 'a') do |csv|
      csv << [titulo.to_s, autor.to_s, edad.to_s, link.to_s, fecha.to_s, respuestas.to_s, imagen.to_s]
    end
  end
...
def parsear(linkparsear)
  ...
  post.css('.single-thread.single-thread-compact-xs.single-thread-compact-sm').each do |pregunta|
    ...
    # puts nrespuestas
    # gets
    pregunta = Pregunta.new(titulo, autor, edad, link, fecha, respuestas, imagen)
    pregunta.guardar
...
CSV.open('viviana_questions.csv', 'wb') do |csv|
  csv << %w[Titulo Autor Edad Link Fecha Respuestas Imagen]
end
